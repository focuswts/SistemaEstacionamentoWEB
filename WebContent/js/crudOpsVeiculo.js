		...
		function() {
			...
			function render(data) {
				...
				var rowH = "<tr> " + "<th>ID</th>" + "<th>Modelo</th>"
						+ "<th>Cor</th>" + "<th>Placa</th>" + "<th>Status</th>"
						+ "<th>Actions</th>" + "</tr>";
				...
				$.each(data, function(index, value) {
					...
					var row1 = $("<td>" + value.idVeiculo + "</td>");

					var row2 = $("<td>" + value.idModelo.descModelo + "</td>");

					var row3 = $("<td>" + value.corVeiculo + "</td>");

					var row4 = $("<td>" + value.placaVeiculo + "</td>");

					var row5 = $("<td>" + value.statusVeiculo + "</td>");
			...
			function setInputData(event) {
				...
				$.each($('#slc_Modelo').children(),
						function(k, v) {
							...
							// é igual
							// Ao valor do OPTION
							if ($(v).text().toLowerCase() === descModelo.toLowerCase()) {
								$(v).prop('selected', true);
							}
						});

				// Define O Valor Do TipoVeiculo No Select
				$.each($('#slc_Cor').children(),
						function(k, v) {
							...
							// é igual
							// Ao valor do OPTION
							if ($(v).text().toLowerCase() === corVeiculo.toLowerCase()) {
								$(v).prop('selected', true);
								$(v).val();
							}
						});
				...
				$.each($('#slc_Status').children(), function(k, v) {
					...
					// é igual
					// Ao valor do OPTION
					if ($(v).text().toLowerCase() === statusVeiculo.toLowerCase()) {
						$(v).prop('selected', true);
					}
				});
				...
				if ($('#tf_Id').val() != "") {
					button.val("Atualizar Veículo");
				} else if ($('#tf_Id').val() == "") {
					button.val("Inserir Veículo");
				}
			...
			function clearFields() {
				...
				$('#slc_Cor').val(0);
				$('#slc_Status').val(0);
				$('#btn-operation').val("Criar Veículo");
			}
			;
			...
			function getInputData(operacao) {
				...
				var idModelo = $('#slc_Modelo').children("option:selected")
						.val();

				var corVeiculo = $('#slc_Cor').children("option:selected")
						.val();

				// Pega O Valor Do Option Status Selecionado
				var statusVeiculo = $('#slc_Status').attr('selected', true)
						.val();
				// alert(statusEstado);
