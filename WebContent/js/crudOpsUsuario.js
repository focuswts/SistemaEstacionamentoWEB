		...
		function() {
			...
			function render(data) {
				...
				$.each(data, function(index, value) {
					...
					var row1 = $("<td>" + value.idUsuario + "</td>");

					var row2 = $("<td>" + value.nomeUsuario + "</td>");

					var row3 = $("<td>" + value.senhaUsuario + "</td>");

					var row4 = $("<td>" + value.statusUsuario + "</td>");
			...
			function setInputData(event) {
				...
				if ($('#tf_Id').val() != null) {
					button.val("Atualizar Usuário");
				} else if ($('#tf_Id').val() == null) {
					button.val("Inserir Usuário");
				}
			...
			function doOperation() {
				...
				if (inputId == "") {
					...
					//Passa o Valor Insert para O Método Decidir Qual Operacao Será feita
